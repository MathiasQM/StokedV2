<template>
  <AppContainer title="Posts" class="mt-20 md:mt-0">
    <template #actions>
    ...
    <UModal
      ...
      :description="
        postModal.isEdit
          ? 'Update your post'
          : 'Create a new post to share with your portfolio'
      "
    >
      <template #body>
        <UForm
...
const createPost = async (post: Partial<InsertPost>) => {
  try {
    const { data, error } = await useFetch<Post>(`/api/super-admin/posts`, {
      method: 'POST',
      body: post,
    })
...
const updatePost = async (id: string, post: Partial<Post>) => {
  try {
    const updatedPost = await $fetch<Post>(`/api/super-admin/posts/${id}`, {
      method: 'PATCH',
      body: post,
    })
    return updatedPost
...
const deletePost = async (id: string) => {
  try {
    ...
    return await $fetch<Post>(`/api/super-admin/posts/${id}`, {
      method: 'DELETE',
    })
  } catch (error: any) {
    toast.add({
