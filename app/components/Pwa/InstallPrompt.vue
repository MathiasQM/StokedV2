    ...
    <DrawerContent class="mb-5 p-4 md:p-6">
      <div class="mx-auto w-full max-w-md space-y-6">
            ...
            <CarouselContent>
              ...
              >
                <img
                  :src="src"
                  alt="App screenshot"
                  class="fade-out-mask select-none"
                  :class="
                    smallerThanMd
        ...
        <div class="space-y-4 px-5">
          ...
          <ol
            class="list-decimal space-y-2 text-black-700 dark:text-white text-sm"
          >
            <li
              v-for="(step, i) in installSteps"
              :key="i"
              class="flex items-center gap-3 text-md"
            >
              <span
                v-if="step.icon"
                class="p-1 bg-gray-200 dark:bg-black-800 rounded-sm flex"
              >
...
import { useBreakpoints, breakpointsTailwind, isIOS } from '@vueuse/core'
...
onMounted(() => {
  ...
  const mobileImages = import.meta.glob(
    '@@/public/screenshots/mobile/*.{png,jpg,jpeg}',
    {
      eager: true,
      import: 'default',
      query: '?url',
  ...
  const desktopImages = import.meta.glob(
    '@@/public/screenshots/desktop/*.{png,jpg,jpeg}',
    {
      eager: true,
      import: 'default',
      query: '?url',
  ...
  const filtered = Object.entries(modules).map(([, url]) => url as string)
  ...
  if (/macintosh/.test(ua)) {
    ...
    installSteps.value = [
      {
        text: 'Tap the menu icon (three dots)',
  ...
  const matchDisplayMode = window.matchMedia?.(
